...
/**
 * Sheet demo page
 * Renders a sheet example with Preview, Code, and Component Call tabs.
 * Includes copy-to-clipboard actions with toast notifications.
 */
const page = () => {
  // State for controlling sheet visibility
  const [isOpen, setIsOpen] = useState(false);

  // States for showing "Copied!" feedback
  const [copied, setCopied] = useState(false);
  const [codeCopi, setCodeCopy] = useState(false);
  const [checkboxCodeInstall, setCheckboxCodeInstall] = useState(false);

  // Installation command for shadcn sheet
  const commandInstall = `npx shadcn@latest add sheet`;

  /**
   * Copy sheet component source code to clipboard
   */
  ...
  /**
   * Copy sheet usage example to clipboard
   */
  ...
  // Configuration object for SheetComponent
  const sheetItem = {
    sheetTrigger: (
  ...
  // Tabs: Preview, Code, and Component Call
  const tabsItem = [
    {
  ...
  // Card content with description, tabs, and extra (install command)
  const mainCard = {
    // title: "Tool Tip",
    description:
